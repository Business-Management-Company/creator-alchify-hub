...
import { FolderOpen, Plus, Clock, Loader2, MoreVertical, Trash2, Wand2, Download, Scissors } from "lucide-react";
  ...
  const fetchProjects = async () => {
    try {
      ...
      if (projectIds.length > 0) {
        const { data: clips } = await supabase.from("clips").select("project_id").in("project_id", projectIds);
        if (clips) {
          clips.forEach((c: any) => {
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Manage your Alchify projects - view, edit, and export your refined content."
        />
      </Helmet>
            ...
            {projects.map((project) => (
              ...
              >
                ...
                <div className="p-5">
                  ...
                  <Button variant="default" className="w-full" onClick={() => navigate(`/refiner/${project.id}`)}>
