  ...
  const { data: podcasts, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data: podcastsData, error: podcastsError } = await supabase
        .from("podcasts")
        .select("id, title, description, image_url, category, language, status, author")
        .eq("status", "active");
      ...
      const podcastsWithEpisodes = await Promise.all(
        (podcastsData || []).map(async (podcast) => {
          ...
          return { ...podcast, episodes: episodes || [], episodeCount, latestEpisode };
        })
      );
      ...
      <div className="min-h-screen bg-gradient-to-br from-background to-muted/20 p-6">
        <div className="max-w-7xl mx-auto p-6">
          <div className="mb-8">
            ...
            <p className="text-muted-foreground mt-1">Discover and listen to amazing podcasts</p>
          </div>
            ...
            {podcasts.map((podcast) => (
              ...
              >
                ...
                <div className="p-4">
                  ...
                  {podcast.latestEpisode && (
                    <div className="border-t pt-3">
                      ...
                      <div className="flex items-center gap-3 text-xs text-muted-foreground">
                        ...
                        {podcast.latestEpisode.pub_date && (
                          <span className="flex items-center gap-1">
                            <Calendar className="w-3 h-3" />
                            {new Date(podcast.latestEpisode.pub_date).toLocaleDateString()}
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  {podcast.author && (
                    <div className="mt-3 text-xs text-muted-foreground">by {podcast.author}</div>
                  )}
                </div>
              </Card>
            ))}
